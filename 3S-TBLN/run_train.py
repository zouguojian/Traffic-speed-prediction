...
class Model(object):
    ...
    def embeddings(self):
        ...
        m_emb = embedding(self.placeholders['minute'], vocab_size=60//self.granularity, num_units=self.para.emb_size, scale=False, scope="minute_embed")
    ...
    def DataGenerator(self):
        self.iterate = DataClass(self.para)

    def re_current(self, a, std, mean):
        return (a * std) + mean

    def run_epoch(self):
        max_mae = 100
        self.sess.run(tf.global_variables_initializer())
        train_next = self.iterate.next_batch(batch_size=self.batch_size, epoch=self.epochs, is_training=True)

        for i in range(int((self.iterate.length * self.divide_ratio - self.total_len)) * self.epochs // self.batch_size):
            ...
            feed_dict = construct_feed_dict(xs, xs_all, self.adj, labels, d_of_week, day, hour, minute, mask=random_mask, placeholders=self.placeholders,site=self.site_num)
            ...
            if i % 100 == 0:
                mae = self.evaluate() # validate processing
                if max_mae > mae:
    ...
    def evaluate(self):
        ...
        if not self.is_training:
            ...
            self.saver.restore(self.sess, model_file)
        test_next = self.iterate.test_batch(batch_size=self.batch_size, epoch=1, is_training=False)
        std_s, mean_s = self.iterate.std_s['speed'], self.iterate.mean_s['speed']
        ...
        for i in range(int((self.iterate.length - self.iterate.length * self.divide_ratio - self.total_len) // self.output_len) // self.batch_size):
        ...
        labels_list = np.reshape(np.array(labels_list, dtype=np.float32),[-1, self.site_num, self.output_len]).transpose([1, 0, 2])
        pres_list = np.reshape(np.array(pres_list, dtype=np.float32),[-1, self.site_num, self.output_len]).transpose([1, 0, 2])
        if self.para.normalize:
            labels_list = self.re_current(labels_list, std_s, mean_s)
            pres_list = self.re_current(pres_list, std_s, mean_s)
