...
def generate_graph_seq2seq_io_data(
    ...
    data = np.reshape(df.values[:,-1:],newshape=[-1, 207, 1]) # [N, nodes, 1]
    ...
    if add_time_in_day:
        ...
        time_in_day = time_in_day.reshape(-1, 207, 1)
...
def generate_train_val_test(args):
    ...
    x_offsets = np.sort(
        ...
        np.concatenate((np.arange(-47, 1, 1),))
    )
    ...
    # split 3/4 samples from source dataset
    # print("x shape: ", x.shape, ", y shape: ", y.shape)
    # total_samples = x.shape[0]
    # x = x[:round(total_samples * (3 / 4))]
    # y = y[:round(total_samples * (3 / 4))]
    print("x shape: ", x.shape, ", y shape: ", y.shape)
