...
def generate_graph_seq2seq_io_data(
    ...
    data = np.reshape(df.values[:,-1:],newshape=[-1, 325, 1]) # [N, nodes, 1]
    ...
    if add_time_in_day:
        ...
        time_in_day = time_in_day.reshape(-1, 325, 1)
...
def generate_train_val_test(args):
    ...
    # split 3/6 samples from source dataset
    # print("x shape: ", x.shape, ", y shape: ", y.shape)
    # total_samples = x.shape[0]
    # x = x[:round(total_samples * (3 / 6))]
    # y = y[:round(total_samples * (3 / 6))]
    print("x shape: ", x.shape, ", y shape: ", y.shape)
