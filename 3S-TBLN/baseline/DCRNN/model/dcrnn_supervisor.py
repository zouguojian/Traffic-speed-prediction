...
class DCRNNSupervisor(object):
    ...
    def evaluate(self, sess, **kwargs):
        ...
        for horizon_i in range(self._data['y_test'].shape[1]):
            ...
            self._logger.info(
                "Horizon {:02d}, MAE: {:.3f}, MAPE: {:.5f}, RMSE: {:.3f}".format(
        ...
        mae = metrics.masked_mae_np(scaler.inverse_transform(y_preds[:y_truth.shape[0], :, :, 0]), scaler.inverse_transform(self._data['y_test'][:y_truth.shape[0], :, :, 0]), null_val=0)
        mape = metrics.masked_mape_np(scaler.inverse_transform(y_preds[:y_truth.shape[0], :, :, 0]), scaler.inverse_transform(self._data['y_test'][:y_truth.shape[0], :, :, 0]), null_val=0)
        rmse = metrics.masked_rmse_np(scaler.inverse_transform(y_preds[:y_truth.shape[0], :, :, 0]), scaler.inverse_transform(self._data['y_test'][:y_truth.shape[0], :, :, 0]), null_val=0)
        self._logger.info(
            "Horizon {:02d}, MAE: {:.3f}, MAPE: {:.5f}, RMSE: {:.3f}".format(
                13, mae, mape, rmse
            )
