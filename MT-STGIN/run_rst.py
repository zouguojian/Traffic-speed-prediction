...
from models.mask import MaskTransformer
...
class Model(object):
    def __init__(self, para):
        ...
        self.placeholders = {
            ...
            'labels_s': tf.placeholder(tf.float32, shape=[None, self.para.site_num, self.para.output_length+self.para.input_length], name='labels_s'),
    ...
    def model(self):
        ...
        # with tf.variable_scope(name_or_scope='mask'):
        #     mask_encoder = MaskTransformer(self.para)
        #     mask_ste = mask_encoder.encoder(inputs=STE[:, self.para.input_length:,:,:])
        #     print('mask ste outs shape is : ', mask_ste.shape)
        ...
        print('#............................in the reconstruct bridge step.............................#')
        with tf.variable_scope(name_or_scope='bridge'):
            # STE[:, :self.para.input_length,:,:]
            # bridge_outs = transformAttention(encoder_outs, encoder_outs, STE[:, self.para.input_length:,:,:], self.para.num_heads, self.para.emb_size // self.para.num_heads, False, 0.99, self.para.is_training)
            ...
            reconstruct_bridge_outs = reconstruct_bridge.encoder(X = tf.reverse(bridge_outs,axis=[1]),
                                                                         ...
                                                                         X_Q = tf.reverse(encoder_outs,axis=[1]))
                                                                         # encoder_outs
            print('reconstruction bridge outs shape is : ', reconstruct_bridge_outs.shape)

        print('#...........................in the reconstruction encoder step..........................#')
        with tf.variable_scope(name_or_scope='encoder', reuse=tf.AUTO_REUSE):
            reconstruct = Encoder_ST(hp=self.para, placeholders=self.placeholders, model_func=self.model_func)
            reconstruct_outs = reconstruct.encoder_spatio_temporal(speed=reconstruct_bridge_outs,
                                                                   STE=tf.reverse(STE[:, :self.para.input_length, :, :],axis=[1]),
                                                                   supports=self.supports)
        ...
        self.loss1 = tf.reduce_mean(tf.sqrt(tf.reduce_mean(tf.square(self.pres_s[:,:,self.para.input_length:] + 1e-10 - self.placeholders['labels_s'][:,:,self.para.input_length:]), axis=0)))
        self.loss2 = tf.reduce_mean(tf.sqrt(tf.reduce_mean(tf.square(self.pres_s[:,:,:self.para.input_length] + 1e-10 - self.placeholders['labels_s'][:,:,:self.para.input_length]), axis=0)))
        loss = 0.7*self.loss1 + 0.3*self.loss2
        self.train_op_1 = tf.train.AdamOptimizer(self.para.learning_rate).minimize(loss)
    ...
    def run_epoch(self):
        ...
        for i in range(int((iterate.length // self.para.site_num * self.para.divide_ratio - (
            ...
            # print(np.transpose(np.squeeze(x_s, axis=-1),[0, 2, 1]).shape, label_s.shape)
            label_s = np.concatenate((np.transpose(np.squeeze(x_s, axis=-1),[0, 2, 1]), label_s),axis=2)
    ...
    def evaluate(self):
        ...
        for i in range(int((iterate_test.length // self.para.site_num
            ...
            label_s_list.append(label_s[:,:,self.para.input_length:])
            pre_s_list.append(pre_s[:,:,self.para.input_length:])
