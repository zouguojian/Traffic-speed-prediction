...
def STEmbedding(SE, TE, T, D, bn, bn_decay, is_training):
    ...
    # spatial embedding
    SE = FC(
        SE, units=[D, D], activations=[None, None],
        bn=bn, bn_decay=bn_decay, is_training=is_training)
    # temporal embedding
    TE = tf.add_n(TE)
    # TE = tf.concat((TE), axis=-1)
    TE = FC(
        TE, units=[D, D], activations=[None, None],
        bn=bn, bn_decay=bn_decay, is_training=is_training)
    return tf.add(SE, TE)
...
def construct_feed_dict(x_s, adj, label_s, day, day_of_week, hour, minute, placeholders):
    ...
    feed_dict.update({placeholders['d_o_w']: day_of_week})
