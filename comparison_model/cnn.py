...
class st_cnn(object):
    def __init__(self, batch_size, predict_time, placeholders=None):
        ...
        self.h=3
        self.w=3
        self.position_size=49
        self.features=5


    def cnn(self,x, position_emb=None):
        '''
        :param x: shape is [batch size,  input length, site num, features]
        :return: shape is [batch size, height, channel]
        '''
        p_emb=position_emb
        p_emb = tf.transpose(p_emb, perm=[0, 2, 1, 3]) # [batch size,  site num, input length, features]
        ...
        res=tf.reshape(res, shape=[cnn_shape[0],cnn_shape[1],nodes])
    ...
    def decoding(self, x, position_emb=None):
        '''
        :param encoder_hs:
        :return:  shape is [batch size, prediction size]
        '''

        pre=self.cnn(x=x, position_emb=position_emb)
