    ...
    def __init__(self, arg):
        ...
        with tf.variable_scope("encoder"):
            ...
            for i in range(self.num_blocks):
                with tf.variable_scope("num_blocks_{}".format(i)):
                    ### Multihead Attention
                    self.enc = multihead_attention(key_emb = self.en_emb, # 是为了保存原始状态，进行0填充的mask
                                                   que_emb = self.en_emb, # 是为了保存原始状态，进行0填充的mask
                                                   queries=self.enc,
                                                    keys=self.enc,
        ...
        with tf.variable_scope("decoder"):
            ...
            for i in range(self.num_blocks):
                with tf.variable_scope("num_blocks_{}".format(i)):
                    ### Multihead Attention
                    self.dec = multihead_attention(key_emb = self.de_emb, # 是为了保存原始状态，进行0填充的mask
                                                   que_emb = self.de_emb, # 是为了保存原始状态，进行0填充的mask
                                                   queries=self.dec,
                                                    keys=self.dec,
                                                    num_units=self.hidden_units,
                                                    num_heads=self.num_heads,
            ...
            for i in range(self.num_blocks):
                with tf.variable_scope("num_blocks_{}".format(i)):
                    ### Multihead Attention
                    self.dec = multihead_attention( key_emb = self.en_emb, # 是为了保存编码部分输入原始状态，进行0填充的mask
                                                    que_emb = self.de_emb, # 是为了保存解码部分输入原始状态，进行0填充的mask
                                                    queries=self.dec,# decoder部分的嵌入
                                                    keys=self.enc,   # encoder部分的嵌入
                                                    num_units=self.hidden_units,
                                                    num_heads=self.num_heads,
                                                    dropout_rate=self.dropout_rate,
                                                    is_training=self.is_training,
