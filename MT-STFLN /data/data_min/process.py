...
new_key=['in_id', 'out_id', 'year', 'month', 'day', 'hour', 'min', 'speed', 'flow']
...
def delete(file_path=None, delete_path=None):
    ...
    key = ['in_id', 'out_id', 'year', 'hour', 'min', 'speed', 'flow']
...
def day_empty(writer=None, id_dict=None, year=2020, month=5, day=1):
    ...
    '''
    for h in range(0, 24):
        for min in range(1,5):
            for id in id_dict:
                line = [id[0], id[1], year, month, day, h, 15*min, 0.0, 0]
                writer.writerow(line)
...
def hour_empty(writer=None, id_dict=None,year=2020, month=5, day=1, hour=0):
    ...
    for min in range(1,5):
        for id in id_dict:
            line = [id[0], id[1], year, month, day, hour, 15*min, 0.0, 0]
            writer.writerow(line)
        ...
        for m in range(5,9):
            ...
            # min
                    for min in range(0,4):
                        data3=data2.loc[data2['min'] == min*15]
                        if data3.values.shape[0]==0:
                            print('min empty')
                            min_empty(writer=writer,id_dict=id_dict,year=year,month=m, day=d,hour=h,min=(min+1)*15)
                            continue
            # road
                        for id in id_dict:
                            data4 = data3.loc[(data3['in_id'] == id[0]) & (data3['out_id'] == id[1])]
                            if data4.values.shape[0] == 0:
                                print('road empty')
                                line = [id[0], id[1], year, m, d, h, (min+1)*15, 0.0, 0]
                                writer.writerow(line)
                                continue

                            print(data4.values)
                            line = [id[0], id[1], year, m, d, h, (min+1)*15, data4.values[-1, -2],data4.values[-1, -1]]
                            writer.writerow(line)
        file.close()
